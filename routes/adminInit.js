...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const [
      ...
      supabase.from('procurements').select('*').order('date', { ascending: false }).limit(500),
      ...
      supabase.from('purchases').select('*').order('date', { ascending: false }).limit(500),
      supabase.from('flour_batches').select('*').order('date', { ascending: false }).limit(200),
      supabase.from('webstore_orders').select('*').order('created_at', { ascending: false }).limit(500),
